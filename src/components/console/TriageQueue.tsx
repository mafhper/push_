...
interface TriageQueueProps {
  ...
  /**
   * Opacity of the text layer for the current width (1 = fully readable).
   * Narrowing the list fades the text out over the last stretch instead of
   * squeezing it, and the rail takes over an already invisible text layer.
   */
  labelOpacity?: number;
  /** Opacity of the rail-only content; the mirror image of `labelOpacity`. */
  railOpacity?: number;
}

export function TriageQueue({ repos, selectedRepoId, compact = false, onToggleCompact, labelOpacity = 1, railOpacity = 1 }: TriageQueueProps) {
  const { t } = useApp();
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col overflow-hidden", compact && "md:items-stretch")}>
      <div className={cn("shrink-0 border-b border-border/50 px-5 py-4 transition-[padding] duration-150 ease-out motion-reduce:transition-none", compact && "md:px-3 md:py-3")}>
        <div className={cn("flex items-start justify-between gap-3", compact && "md:justify-center")}>
          <div className={cn("min-w-0", compact && "md:hidden")} style={{ opacity: labelOpacity }}>
            {/* Truncating instead of wrapping: a wrapped header is what made the
                list look squeezed while it was still being narrowed. */}
            <h2 className="truncate text-micro font-bold uppercase tracking-wider text-foreground-subtle">{t('repositoryTriage')}</h2>
            <p className="mt-1 truncate text-[10px] text-foreground-subtle/70">
        ...
        <p
          className={cn("hidden text-center font-mono text-[10px] font-semibold text-primary", compact && "md:block")}
      ...
      <div
        className={cn("flex shrink-0 items-center gap-1.5 overflow-x-auto border-b border-border/50 px-4 py-2.5", compact && "md:hidden")}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        >
          ...
          <span className={cn("min-w-0 flex-1 truncate text-sm font-semibold text-foreground", compact && "md:hidden")} style={{ opacity: labelOpacity }}>
            {t('dashboardOverview')}
          </span>
        </button>
...
function TriageRow({ repo, selected, compact, labelOpacity, railOpacity, onClick }: { repo: ScoredRepo; selected: boolean; compact: boolean; labelOpacity: number; railOpacity: number; onClick: () => void }) {
  const { t } = useApp();
  ...
  return (
    ...
    >
      {/* Only the avatar resizes at the threshold, so only its box animates. */}
      <div className={cn("flex shrink-0 items-center gap-2 transition-[width,height] duration-150 ease-out motion-reduce:transition-none", compact && "md:block")}>
        ...
        <span className={cn(compact && "md:hidden")} style={{ opacity: labelOpacity }}>
          <SeverityDot severity={severity} label={severityLabel} />
        </span>
      </div>

      <div className={cn("flex min-w-0 flex-1 flex-col", compact && "md:hidden")} style={{ opacity: labelOpacity }}>
        <span className={cn(
      ...
      <div className={cn("flex shrink-0 flex-col items-end gap-0.5", compact && "md:flex-row md:items-center md:justify-center md:gap-1")}>
        <span className={cn("hidden", compact && "md:inline-flex")} style={{ opacity: railOpacity }}>
          <SeverityDot severity={severity} label={severityLabel} />
        </span>
        <span className={cn(
