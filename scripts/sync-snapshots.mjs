...
async function loadEnvFile() {
  ...
  try {
    const content = await fs.readFile(normalizePath(envPath), "utf8");
...
async function main() {
  ...
  for (const entry of config.repositories) {
    ...
    const [repoPayload, commitsPayload, workflowsPayload, languagesPayload, contributorsPayload, dependabotPayload] = await Promise.all([
      fetchOptional(repoPath, token),
      ...
      token ? fetchOptional(`${repoPath}/dependabot/alerts?state=open&per_page=20`, token) : Promise.resolve({ error: "Dependabot alerts require authenticated access." }),
    ]);
    ...
    const workflowRuns = Array.isArray(workflowsPayload?.workflow_runs) ? workflowsPayload.workflow_runs.map(mapWorkflowRun) : [];
    ...
    const availability = {
      ...
      commits: Array.isArray(commitsPayload) ? createAvailability(true, "public-api") : createAvailability(false, "public-api", commitsPayload.error),
      ...
      languages: !languagesPayload?.error ? createAvailability(true, "public-api") : createAvailability(false, "public-api", languagesPayload.error),
    ...
    await fs.writeFile(
      path.join(OUTPUT_REPOS, fileName),
  ...
  const manifest = {
    site: config.site,
    status,
    featuredRepo: config.featuredRepo,
    routes: {
